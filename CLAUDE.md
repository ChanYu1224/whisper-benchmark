...
| `bench_gemma.py` | Gemma (テキスト生成 LLM) の推論ベンチ。RTF ではなく TTFT / decode tok/s / e2e tok/s / VRAM を計測。`--quant fp16\|bf16\|int8\|int4` で条件切替 (int8/int4 は bitsandbytes)。依存追加: `transformers accelerate bitsandbytes`。Gemma は gated なので HF トークン + ライセンス同意が必要。 |
| `compare.py` | `results/*.json` を集計しマシン横断の比較表 (Markdown) を生成。`--out COMPARISON.md`。Whisper と Gemma 両方に対応。 |
| `plot.py` | `results/*.json` からマシン横断の比較グラフ (PNG) を生成。Whisper は `charts/benchmark_comparison.png`、Gemma は `charts/gemma_comparison.png`。matplotlib 必要。 |
| `results/` | `<benchmark>_<machine>_<model>[_<variant>].json`。同一キーは最新で上書き。Gemma は `variant` に quant (fp16/int8 等) が付く。 |
